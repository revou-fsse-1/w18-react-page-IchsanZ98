...
export default function JoinForm(props: {
  ...
  const [formFieldsError, setFormFieldsError] = useState<formFieldsError>(
    INITIAL_FORM_FIELD_ERROR
  );
  ...
  const setFormFieldErrorByFieldKey = (
    fieldkey: string,
    errorMessage: string
  ) => {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!formFields.fiirstName) {
      setFormFieldErrorByFieldKey(
        "fiirstName",
        "First Name have to be filled in"
      );
    }
    ...
    if (!formFields.email || !formFields.fiirstName || !formFields.lastName)
      return;
        ...
        <div className="relative bg-white px-12 py-6 rounded-lg max-w-[600px]">
          ...
          <div className="flex flex-col items-start">
            <h1 className="font-bold text-2xl text-slate-800">
              Register to Join Unique Gallery Membership
            </h1>
            <p className="text-sm text-gray-600 mb-4">
              Get countless benefit by joining as a member of Unique Gallery!
            </p>
          </div>

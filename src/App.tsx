...
function App() {
  ...
  return (
    <div className="flex flex-col items-center justify-center">
      ...
      <div className="mt-4 max-w-[1200px] flex flex-wrap flex-row justify-center items-center gap-6">
        {galleries
          .filter((gallery) => {
      ...
      <button
        ...
        className=" mt-6 cursor-pointer border-[1px] border-grey z-0 px-6 py-1 text-md font-semibold top-[3px] right-[3px] bg-white rounded-lg hover:scale-110 duration-200"
      >
        Join Unique Gallery Membership
      </button>
      {isShow ? (
        <JoinForm setIsShow={setIsShow} setIsActive={setIsActive} />
      ) : null}
      {isActive ? (
        <JoinNotification setIsActive={setIsActive} isActive={isActive} />
      ) : null}
    </div>
  );
}
